...
float process_pair(
  const float NTN_W[nFilters3][nFilters3][nTensorNeurons],
  const float NTN_WB[nTensorNeurons][nFilters3*2],
  const float NTN_bias[nTensorNeurons],
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
  ...
  #pragma HLS array_partition variable=NTN_W cyclic dim=2 factor=2
  #pragma HLS array_partition variable=NTN_W complete dim=3
  ...
  for(int row = 0; row < nrow; row ++){
    ...
    for(int col = 0; col < ncol; col ++){
      #pragma HLS pipeline
      #pragma HLS LOOP_TRIPCOUNT max=1200 min=1200
      result_cache[col] = process_pair(NTN_W, NTN_WB, NTN_bias, L1_W, L1_bias, L2_W, L2_bias, L3_W, L3_bias, re[row], ce[col]);
    }
